...
public class PacManGameManager : MonoBehaviour
{
    ...
    [Header("Win Settings")]
    [SerializeField] private int targetScore = 2000;
    ...
    private bool pacmanCompleted = false;
    ...
    void SpawnGhostAtIndex(int index)
    {
        ...
        GameObject ghostObj = Instantiate(
            ghostPrefabs[index],
            ghostSpawnPoints[index].position,
            Quaternion.identity
        );
        ...
        {
            Debug.LogError($"Ghost prefab {ghostPrefabs[index].name} has no GhostGridMover!");
        }
    }
    ...
    {
        if (ghostIndex < 0 || ghostIndex >= ghostPrefabs.Length)
            return;
        ...
        respawnCoroutines[ghostIndex] =
            StartCoroutine(RespawnGhostAfterDelay(ghostIndex));
    }
    ...
    public void AddScore(int points)
    {
        ...
        // Победа по очкам
        if (score >= targetScore && !pacmanCompleted)
        {
            pacmanCompleted = true;

            GameManager.Instance.GivePacmanKey();

            Debug.Log("Pacman key obtained!");
    ...
    void ShowFloatingText(Vector3 pos, int value)
    {
        ...
        {
            Debug.LogWarning("floatingTextPrefab missing!");
